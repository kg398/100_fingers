...
union(){
    ...
    rotate([0,0,0]){
        //index finger
        translate(o_index) rotate([0,0,-print_ang]) rotate([0,-10,0])
            finger(bl_index,jd_index,ang=print_ang,index=true,pulley=p_index);
        //middle finger
        translate(ro_middle) rotate([0,0,-print_ang]) rotate([0,00,0])
            finger(bl_middle,jd_middle,ang=print_ang,pulley=p_index);
        //ring finger
        translate(ro_ring) rotate([0,0,-print_ang]) rotate([0,10,0])
            finger(bl_ring,jd_ring,ang=print_ang,pulley=p_index);
        //little finger
        translate(ro_little) rotate([0,0,-print_ang]) rotate([0,20,0])
