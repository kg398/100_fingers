...
module modular_bone(config,bl,jd,w,lig=[2.5,0.6],p=[5,1.5,1],ang=45,thumb_adj1=0,thumb_adj2=0,thumb_side=0){
    ...
    union(){
        ...
        if(config[4]==1 || config[4]==3){ //joint 1
            ...
            trans2 = config[1]==2 ? [bl-jd[1]/2+p[2]-p[0],jd[1]/2,-w[1]/2]:[bl-jd[1]/2+p[2]-p[0],jd[1]/2,-w[1]/2];
...
//bone(34.3,15.6, 7.1,5.5, w2=15.4,bone_type=inte,joint_type=0,lig_ang=45);
//multi-bone module
//l -> length of tendon
...
module bone_old(l, d1, d2, w, w2=0, bone_type=0, joint_type=0, lig_type=0, lig_ang=0, pulley_type=0, tendon_type=1, tl=[100,100], nail_type=0, en=false, center=true, lw=1.6, lt=0.6, pl=5, pt=1.5, pr=1, of=false, thumb=0){
    ...
    //------------------------------proximal------------------------------//
        ...
        union(){
            ...
            difference(){
                ...
                if(thumb!=0){
                    translate([l/2+pl/2+0.5,d1/2,-(w+w2)/4]){ rotate([0,0,0])
                        cylinder(4.5,1,1);
                        chamfer(2,h=1);
                    }
                }
            ...
            if(pulley_type==0){
                ...
                if(thumb!=0){
                    //abd1
                    translate([l/2-pl/2,d1/2,-(w+w2)/4]) rotate([-90,0,0])
                        pulley(0.75*pl,pr,pt,o_fillet=false,t_fillet=true);
                }
            }
        }
    //------------------------------intermediate------------------------------//
                ...
                union(){
                    ...
                    if(nail_type==0){
                        translate([l/2-d2/2,-1,-w2/2]){
                            translate([0,-0.1,0]) cube([l/2,1.2,w2]);
                            translate([0,-2,-1]){
                                difference(){
                                    cube([l/2+3,2,w2+2]);
                                    translate([l/2+3,1.3,-0.1]) rotate([0,0,45])
                                        cube([1,1,w2+2.2]);
                                }
                            }
                        }
