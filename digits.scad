...
module finger(bone_lengths, joint_diams, bone_width=10, jt=0, ang=0, index=0,pulley=[5,1.5,1]){
    ...
    rotate([0,0,ang]){
        union(){
            ...
            translate([bone_lengths[0]-joint_diams[0]+(0.5*PI*joint_diams[0]+lt)*cos(ang),-joint_diams[0]+lt+0.2-(0.5*PI*joint_diams[0]+lt)*sin(ang),0]){
                //proximal
                bone(bone_lengths[1],joint_diams[0],joint_diams[1],bone_width,bone_type=prox,joint_type=1,lig_ang=ang,pl=pulley[0], pt=pulley[1], pr=pulley[2]);
...
//jt -> joint types unused {0}->{default}
//ang -> ligament angle, use for printing bones in different orientations
module thumb(bone_lengths, joint_diams, bone_width=15, jt=0, ang=0, thumb=1, pulley=[5,2.2,1]){
    ligament_width=1.6;//default ligament width
    ligament_thickness=0.6;//default ligament thickness
    rotate([0,0,ang]){
        union(){
            //carpal
            bone(bone_lengths[0],joint_diams[0],joint_diams[0],bone_width,bone_type=carp,joint_type=jt,lig_ang=ang,lw=ligament_width,lt=ligament_thickness,pl=pulley[0], pt=pulley[1], pr=pulley[2]);
            //relative translate from mcp geometry
            translate([bone_lengths[0]-joint_diams[0]+(0.5*PI*joint_diams[0]+ligament_thickness)*cos(ang),-joint_diams[0]+ligament_thickness+0.2-(0.5*PI*joint_diams[0]+ligament_thickness)*sin(ang),0]){
                ...
                translate([bone_lengths[1]-joint_diams[1]+(0.5*PI*joint_diams[1]+ligament_thickness)*cos(ang),-joint_diams[1]+ligament_thickness+0.2-(0.5*PI*joint_diams[1]+ligament_thickness)*sin(ang),0]){
                    ...
                    translate([bone_lengths[2]-joint_diams[2]+(0.5*PI*joint_diams[2]+ligament_thickness)*cos(ang),-joint_diams[2]+ligament_thickness+0.2-(0.5*PI*joint_diams[2]+ligament_thickness)*sin(ang),0]){
                        //distal
                        bone(bone_lengths[3],joint_diams[2],joint_diams[2],bone_width,bone_type=dist,joint_type=jt,lig_ang=ang,pl=pulley[0], pt=pulley[1], pr=pulley[2]);
                    }
                }
            }
        }
...
//finger module
module finger_nolig(bone_lengths, joint_diams, bone_width=10, jt=0, ang=0, index=0,pulley=[5,1.5,1]){
    lw=1.6;//default ligament width
    lt=0.6;//default ligament thickness
    rotate([0,0,ang]){
        union(){
            ...
            bone(bone_lengths[0],joint_diams[0],joint_diams[0],bone_width,bone_type=meta,joint_type=jt,lig_ang=ang,pl=pulley[0], pt=pulley[1], pr=pulley[2]);
            //relative translate from mcp geometry
            translate([bone_lengths[0]+lt,0,0]){
                //proximal
                ...
                //relative translate from pip geometry
                translate([bone_lengths[1]+lt,0,0]){
                    //intemediate
                    ...
                    //relative translate from dip geometry
                    translate([bone_lengths[2]+lt,0,0]){
                        //distal
...
//thumb module
module thumb_nolig(bone_lengths, joint_diams, bone_width=15, jt=0, ang=0, thumb=1, pulley=[5,2.2,1]){
    ligament_width=1.6;//default ligament width
    ligament_thickness=0.6;//default ligament thickness
    rotate([0,0,ang]){
        union(){
            ...
            bone(bone_lengths[0],joint_diams[0],joint_diams[0],bone_width,bone_type=carp,joint_type=jt,lig_ang=ang,lw=ligament_width,lt=ligament_thickness,pl=pulley[0], pt=pulley[1], pr=pulley[2]);
            //relative translate from mcp geometry
            translate([bone_lengths[0]+ligament_thickness,0,0]){
                //proximal
                difference(){
                    bone(bone_lengths[1],joint_diams[0],joint_diams[1],bone_width,bone_type=prox,joint_type=1,lig_ang=ang,pl=pulley[0], pt=pulley[1], pr=pulley[2],lig_type=1);
                    if(thumb!=0){
                ...
                //relative translate from pip geometry
                translate([bone_lengths[1]+ligament_thickness,0,0]){
                    //intemediate
                    bone(bone_lengths[2],joint_diams[1],joint_diams[2],bone_width,bone_type=inte,joint_type=jt,lig_ang=ang,pl=pulley[0], pt=pulley[1], pr=pulley[2],lig_type=1);
                    //relative translate from dip geometry
                    translate([bone_lengths[2]+ligament_thickness,0,0]){
                        //distal
